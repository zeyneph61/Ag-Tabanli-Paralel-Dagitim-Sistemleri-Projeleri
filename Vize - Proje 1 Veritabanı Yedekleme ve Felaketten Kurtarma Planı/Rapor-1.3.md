...
Point-in-Time Restore bir veri tabanını belirli bir tarih ve saate geri döndürme yöntemidir. Yanlışlıkla silinen veya bozulan verilerin tam olarak hangi anda kaybolduğu biliniyorsa, o anı hedef alarak kurtarma yapılabilir. Bu yöntem yalnızca Recovery Model'i FULL olan veri tabanlarında ve Transaction Log yedekleri mevcut olduğunda kullanılabilir.

Adım 1 - Test verisi oluşturuldu:

``` sql
USE Northwind;
CREATE TABLE PITRTest (ID INT, Mesaj NVARCHAR(100));
INSERT INTO PITRTest VALUES (1, 'deneme');
SELECT * FROM PITRTest;
```

![PITR1](gorseller/gorsel19.png)

Adım 2 - Full Backup alındı:

``` sql
BACKUP DATABASE Northwind
TO DISK = 'C:\NorthwindBackups\Northwind_PITR_Full.bak'
WITH FORMAT, NAME = 'Northwind PITR Full Backup';
```

![PITR2](gorseller/gorsel20.png)

Adım 3 - Geri yükleme için hedef zaman kaydedildi:

``` sql
SELECT GETDATE();
``` 

Sorgu sonucu 2026-04-09 21:33:54 olarak kaydedilmiştir. Bu değer STOPAT parametresi olarak kullanılacaktır.

![PITR3](gorseller/gorsel21.png)


Adım 4 - Transaction Log Backup alındı:

```sql
BACKUP LOG Northwind
TO DISK = 'C:\NorthwindBackups\Northwind_PITR_Log.bak'
WITH NAME = 'Northwind PITR Log Backup';
```

![PITR4](gorseller/gorsel22.png)


Adım 5 - Felaket simüle edildi (tablo silindi):
...
![PITR4](gorseller/gorsel23.png)

Adım 6 - Point-in-Time Restore gerçekleştirildi:

STOPAT parametresine 3. adımda kaydedilen zaman damgası girilmiştir. Bu sayede veri tabanı tam olarak o ana geri döndürülmüştür.
...
RESTORE DATABASE Northwind
FROM DISK = 'C:\NorthwindBackups\Northwind_PITR_Full.bak'
WITH NORECOVERY, REPLACE;

RESTORE LOG Northwind
FROM DISK = 'C:\NorthwindBackups\Northwind_PITR_Log.bak'
WITH STOPAT = '2026-04-09 21:33:54', RECOVERY;

ALTER DATABASE Northwind SET MULTI_USER;
```

![PITR5](gorseller/gorsel24.png)


Adım 7 - Verinin geri geldiği doğrulandı:
...
![PITR6](gorseller/gorsel25.png)


Sonuç: PITRTest tablosu ve içindeki veri başarıyla geri getirilmiştir. Bu test Transaction Log yedeklemesi ve STOPAT parametresi kullanılarak veri tabanının saniye bazında belirli bir ana geri döndürülebildiğini somut olarak ortaya koymuştur.
...
Point-in-Time Restore senaryosunda ise yalnızca belirli bir ana geri dönmenin mümkün olduğu gösterilmiştir. Transaction Log yedeklemesi ve STOPAT parametresi kullanılarak, yanlışlıkla silinen PITRTest tablosu saniye bazında hassasiyetle kurtarılmıştır. Bu yöntem, özellikle tam olarak ne zaman veri kaybı yaşandığının bilindiği durumlarda full backup'a kıyasla çok daha hedefli bir kurtarma imkânı sunmaktadır.
