...
- [2. Performans İzleme](#2-performans-i̇zleme)
  ...
  - [2.2 SQL Profiler ile İzleme](#22-sql-profiler-ile-i̇zleme)
  - [2.3 Test Sorgusu Oluşturulması](#23-test-sorgusu-oluşturulması)
  - [2.4 Execution Plan Analizi](#24-execution-plan-analizi)
...
- [5. Disk Alanı Yönetimi](#5-disk-alanı-yönetimi)
  - [5.1 Veritabanı Dosya Boyutlarının İncelenmesi](#51-veritabanı-dosya-boyutlarının-i̇ncelenmesi)
  - [5.2 Tablo Bazında Disk Kullanımı](#52-tablo-bazında-disk-kullanımı)
  - [5.3 SHRINK ile Alan Geri Kazanımı](#53-shrink-ile-alan-geri-kazanımı)
- [6. Sonuç](#6-sonuç)
...
Bu doğrultuda aşağıdaki adımlar uygulanmıştır:
  ...
  •	DMV kullanılarak mevcut sorgu performansı izlenmiştir.
  •	SQL Server Profiler ile gerçek zamanlı sorgu izleme gerçekleştirilmiştir.
  •	Test sorgusu oluşturularak Execution Plan analizi yapılmıştır.
  •	Mevcut indexler incelenmiş, eksik ve gereksiz indexler tespit edilmiştir.
  •	Composite index oluşturularak yüksek maliyetli Anahtar Arama operasyonu ortadan kaldırılmıştır.
  •	Duplicate indexler silinerek index yapısı sadeleştirilmiştir.
  •	Sorgu yapısı iyileştirilerek erken filtreleme sağlanmıştır.
  •	Disk alanı analizi yapılmış, SHRINK işlemiyle gereksiz alan geri kazanılmıştır.
...
## 2.2 SQL Profiler ile İzleme

DMV analizinin ardından SQL Server Profiler aracı kullanılarak veritabanı üzerinde gerçek zamanlı sorgu izleme gerçekleştirilmiştir. Profiler; sorguların CPU kullanımı, okuma sayısı ve çalışma süresi gibi metriklerini canlı olarak takip etmeye imkân tanımaktadır.

İzleme başlatıldıktan sonra Northwind veritabanı üzerinde üç farklı test sorgusu çalıştırılmıştır:
...
İzleme tamamlandıktan sonra trace durdurulmuştur. 

![İzlemeDurduruldu](gorseller/gorsel12.png)



## 2.3	Test Sorgusu Oluşturulması
...
## 5.1 Veritabanı Dosya Boyutlarının İncelenmesi

Veritabanının disk üzerinde kapladığı alanı incelemek amacıyla sys.database_files görünümü sorgulanmıştır.
...
Sorgu sonucunda Northwind veri dosyasının 72 MB, log dosyasının ise 8 MB disk alanı kapladığı görülmüştür. Growth değerinin 8192 olması, her otomatik büyümede dosyanın 64 MB genişleyeceğini göstermektedir. 

![DiskBoyutu](gorseller/gorsel13.png)


## 5.2 Tablo Bazında Disk Kullanımı

Hangi tabloların ne kadar alan kullandığını tespit etmek amacıyla aşağıdaki sorgu çalıştırılmıştır.
...
Sonuçlar incelendiğinde Orders tablosunun toplam alanının yaklaşık %45'inin, Employees tablosunun ise büyük bölümünün kullanılmadığı tespit edilmiştir.

![DiskBoyutu2](gorseller/gorsel14.png)

## 5.3 SHRINK ile Alan Geri Kazanımı

Kullanılmayan disk alanını geri kazanmak amacıyla DBCC SHRINKDATABASE komutu çalıştırılmıştır.
...
İşlem sonucunda Northwind veri dosyasının boyutu 72 MB'dan 9 MB'a düşmüştür. Bu işlem sayesinde veritabanının kapladığı alan yaklaşık %87 oranında azaltılmıştır. 

![DiskBoyutu3](gorseller/gorsel15.png)
...
Bu proje kapsamında Northwind veritabanı üzerinde performans analizi ve optimizasyon çalışmaları gerçekleştirilmiştir. Yapılan çalışmalar sonucunda aşağıdaki iyileştirmeler sağlanmıştır:
  ...
  •	SQL Server Profiler ile gerçek zamanlı sorgu izleme gerçekleştirilmiş; CPU, okuma sayısı ve süre metrikleri takip edilmiştir.
  ...
  •	Disk alanı analizi yapılmış, SHRINK işlemiyle veritabanı boyutu 72 MB'dan 9 MB'a düşürülmüştür.

Çalışma boyunca SQL Server'ın sunduğu DMV'ler, Profiler ve Execution Plan araçlarının performans sorunlarının tespitinde ne denli etkili olduğu görülmüştür.
